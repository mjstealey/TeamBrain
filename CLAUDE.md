...
**Phase 5 § B — Slack `/tb` slash command — built 2026-06-11; live smoke pending two Michael-driven steps.** A `/tb` slash command (`remember`/`recall`/`recent`/`status`/`link`/`help`) scoped per channel to a project via `migrations/0023_slack_channels.sql` (service_role-only mapping table) and `edge-functions/teambrain-slack/` (the 8th function). Deliberate divergence from OB1's slack-capture: explicit capture only (no message-inbox; § C over-capture lesson), and writes ride a 5-min bot JWT through `teambrain-rest` — same RLS + 0012 capability fence (`project` scope only, capture+read), never service_role. Authn = Slack HMAC signature (`SLACK_SIGNING_SECRET`, the app's only credential — slash-command-only app, no bot token); the dispatcher's `VERIFY_JWT` gate is satisfied by nginx injecting the public anon key on the webhook path only. Admin link routes (`POST/GET/DELETE /links*`, GitHub-OAuth JWT + project-admin gate) are in the OpenAPI spec under the `slack` tag; adoption kit `examples/slack/` (manifest + README); runbook `deploy/production/README.md` § 11c (incl. a synthetic signed smoke needing no Slack app). **Remaining: B5 (create the Slack app from `examples/slack/manifest.yml`, set the signing secret on the VM, deploy) and B6 (in-Slack end-to-end smoke)** — see `docs/phase-5-checklist.md` § B. After that, Phase 5 is complete and the "connect from every surface" docs trigger fires (`docs/documentation-plan.md` § 3); Phase 7 pilot remains unblocked.
