...
## C — Commit-triggered staleness flagging — 🟡 *code-complete on `feat/phase6-staleness-flagging`; production apply + smoke pending*
...
**Done when:** a push touching a pinned path surfaces the affected thought(s) as needing re-verification, through the pluggable signal path; no false-positive flag on an unrelated path. Covered by `scripts/smoke-staleness-flagging.md` (pin two thoughts to different paths, fire the core for one path, assert exactly the matching thought is flagged + surfaced, the control is untouched, and re-verify clears it). ☐ **Remaining (operator):** confirm the GitHub App has **`Contents: read`**; apply `0018` (+ `NOTIFY pgrst`) and prod `0019`; deploy `teambrain-staleness` + the patched `teambrain-mcp`/`teambrain-rest`; run the smoke on `pr.fabric-testbed.net`.
