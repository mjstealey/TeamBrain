---
description: Capture a memory to TeamBrain (search-first dedup, then capture)
...
a mis-routed thought can land in the wrong project.
...
2. Otherwise call `capture_project_thought` with: `project_slug`; `content` (the memory —
   ...
   the human said so); `tags` including `slash-capture`; `paths` if clear.
