...
- [ ] **GitHub Sync App (production).** Registered as `TeamBrain Sync — fabric-testbed` (no `(dev)` suffix). Permissions: Repository → Metadata: Read, **Contents: Read & write, Pull requests: Read & write, Workflows: Read & write**; Organization → Members: Read. (Workflows is separate from Contents and is what lets `teambrain-console` setup-pr commit `.github/workflows/` files — see `docs/deployment.md` § "Repository permissions". Remember to **accept** any permission bump on the installation.) Webhook disabled. Installed on the org with the pilot repo selected. App ID + installation ID + PKCS#8-converted private key captured. Distinct from the scratch Sync App per `docs/deployment.md` § "Scratch vs production: register two Apps".
